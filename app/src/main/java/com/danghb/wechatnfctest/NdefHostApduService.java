...
public class NdefHostApduService extends HostApduService {
    ...
    private static final byte[] INS_NOT_SUPPORTED = hex("6D00");
    private static final byte[] CC_FILE = hex("000F2000FF00FF0406E1047FFF00FF");
    ...
    public byte[] processCommandApdu(byte[] apdu, Bundle extras) {
        ...
        if (ins == 0xA4) {
            ...
            if (p1 == 0x04 && Arrays.equals(data, NDEF_AID)) {
                if (!isEnabled()) return FILE_NOT_FOUND;
        ...
        if (ins == 0xB0) {
            ...
            int le = readLe(apdu);
            if (le <= 0) le = 256;
            byte[] file = selectedFile == FILE_CC ? CC_FILE : buildNdefFile();
            if (offset > file.length) return WRONG_PARAMS;
            int end = Math.min(file.length, offset + le);
            byte[] part = Arrays.copyOfRange(file, offset, end);
            return concat(part, OK);
        }
    ...
    private byte[] buildNdefFile() {
        ...
        byte[] file = new byte[2 + ndef.length];
        file[0] = (byte) ((ndef.length >>> 8) & 0xFF);
        file[1] = (byte) (ndef.length & 0xFF);
